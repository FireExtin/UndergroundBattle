...
sequenceDiagram
    ...
    Game->>Func: 刷新地区控制
    Func->>Func: resolveBaseRegionInfluence()
    Note right of Func: 获取 BaseInfluenceByPlayer<br/>或使用旧的 InfluenceByPlayer
    
    Func->>Char: 遍历桌上的角色卡
    Char-->>Func: 返回准备好的角色影响力
    Note right of Func: deriveReadyCharacterRegionInfluence()<br/>只统计非耗尽的角色
...
| 函数名 | 作用 | 逻辑说明 |
|--------|------|----------|
| `resolveBaseRegionInfluence()` | 解析基础影响力 | 优先使用 `BaseInfluenceByPlayer`，否则使用旧的 `InfluenceByPlayer` |
...
**变更内容：**
在添加影响力计数器时，同时更新 `BaseInfluenceByPlayer` 和 `InfluenceByPlayer`。
...
| `CardState` | `RegionInfluenceDerived` | `bool` | 标记是否包含动态派生的影响力 |
