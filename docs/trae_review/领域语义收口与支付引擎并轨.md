...
- ✅ **支付引擎并轨**: 正式 battle 动作已开始统一到 `PaymentEngine` 边界；当前稳定范围是 `play_card` / `build_asset`
- ✅ **构筑规则启用**: 默认启用派系限制（最多2个）、牌库大小（40-60张）、同名副本（最多3张）验证
- ✅ **视觉表现增强**: 卡牌显示战斗/防御/调查数值，支持横置/暗置/摧毁状态的视觉反馈

> 2026-04-03 修正：本文原先把 `queue_operation` 也写成正式支付/忠诚入口，这已经被回退。当前实现中，`session.lifecycle` 是唯一内部状态源；`SetupState.Lifecycle` 仅在 API 响应时投影；`queue_operation` 保持调试/fixture 通道定位，不强行套用正式 battle 支付与忠诚规则。
...
**变更说明**: 废弃了由多个布尔值拼凑生命周期的做法，引入显式 `SessionLifecycle` 状态机，所有状态转移必须通过统一的 `Transition()` 方法。当前内部只维护 `session.lifecycle`；`SetupState.Lifecycle` 仅是 API 投影字段。
...
**变更说明**: 确立 `PaymentEngine` 接口，将正式 battle 动作逐步纳入统一支付预检和扣费流程。当前稳定范围是 `play_card` / `build_asset`；`queue_operation` 继续保留调试/fixture 通道兼容。隔离 `Prototype`（当前原型）与 `Rulebook`（未来正式规则）模式。
...
| 支付引擎并轨 | 正式 battle 动作（如 `play_card` / `build_asset`） | 确保卡牌 `Cost` 字段正确设置 |
...
2. ✅ **费用不足**: 验证正式 battle 动作（如 `play_card` / `build_asset`）在资源不足时被拒绝
